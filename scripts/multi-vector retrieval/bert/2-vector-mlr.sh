...
accelerate launch --multi_gpu --mixed_precision=no --num_processes=4 \
	...
	--learner_type "dm" --untie_query_encoder --layers "[12,8]" \
	--num_warmup_steps 1237 --weight_decay 0.0 --max_grad_norm 2.0 \
	--data_dir "../data/openqa" \
	--output_dir "results_squad1/dmde2_1208_lr5_e40" \
	...
	> run_dmde2_1208_lr5_e40.out 2>&1
...
for _id in $(seq 0 $((${NUM_SHARDS}-1))); do
	...
	--num_machines=1 --dynamo_backend=no -m proj_dense.train_dense_retriever \
		--task "squad1" \
		--model_name_or_path "results_squad1/dmde2_1208_lr5_e40" \
		--learner_type "dm" --untie_query_encoder --layers "[12,8]" \
		--data_dir "../data/openqa" \
		--output_dir "results_squad1/dmde2_1208_lr5_e40" \
		--seed 12345 \
		...
		--max_length 256 \
		--eval_batch_size 128  \
		> pred_dmde2_1208_lr5_e40_${_id}.out 2>&1
done	
...
accelerate launch --multi_gpu --mixed_precision=no --num_processes=4 \
	...
	--eval_batch_size 128 \
	--hits_num 100 \
	> pred_dmde2_1208_lr5_e40.out 2>&1