...
accelerate launch --multi_gpu --mixed_precision=no --num_processes=4 \
	...
	--learner_type "t5-dm" --untie_query_encoder --layers "[12,11,10,9,8,7]" \
	--num_warmup_steps 1237 --weight_decay 0.0 --max_grad_norm 2.0 \
	--data_dir "../data/openqa" \
	--output_dir "results_squad1/t5_dmde6_last6_lr4_e40" \
	...
	> t5_run_dmde6_last6_lr4_e40.out 2>&1
...
for _id in $(seq 0 $((${NUM_SHARDS}-1))); do
	...
	--num_machines=1 --dynamo_backend=no -m proj_dense.train_dense_retriever \
		--task "squad1" \
		--model_name_or_path "results_squad1/t5_dmde6_last6_lr4_e40" \
		--learner_type "t5-dm" --untie_query_encoder --layers "[12,11,10,9,8,7]" \
		--data_dir "../data/openqa" \
		--output_dir "results_squad1/t5_dmde6_last6_lr4_e40" \
		--seed 12345 \
		...
		--max_length 256 \
		--eval_batch_size 128  \
		> t5_pred_dmde6_last6_lr4_e40_${_id}.out 2>&1
done	
...
accelerate launch --multi_gpu --mixed_precision=no --num_processes=4 \
	...
	--eval_batch_size 128 \
	--hits_num 100 \
	> t5_pred_dmde6_last6_lr4_e40.out 2>&1