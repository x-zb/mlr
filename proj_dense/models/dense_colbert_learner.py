...
from peach.enc_utils.enc_learners import ColBERTLearnerMixin
...
class ColBERTDenseLearner(ColBERTLearnerMixin):
    ...
    def forward(
        ...
        # x = (torch.sum(attention_mask,dim=1)==0).nonzero()
        # if x.shape!=torch.Size([0,1]):
        #     logger.info(f'query: {input_ids_query},\ndoc lengths: {torch.sum(attention_mask,dim=1)}\ndoc: {input_ids},\nzero_doc({x[0][0]}): {input_ids[x[0][0],:]}')
        return emb_doc,emb_query,attention_mask # (bsz*num_doc,n_prompt,hidden_size),(bsz,hidden_size),(bsz*num_doc,doc_len)
        
    
    def compute_loss(self, emb_doc, emb_query, attention_mask):
        # (bsz*num_doc,n_prompt,hidden_size), (bsz,hidden_size), (bsz*num_doc,doc_len)
        ...
        if self.model_args.do_xentropy:
            ...
            if self.model_args.pooling!='mean':
                ga_attention_mask = self.gather_tensor(attention_mask.unsqueeze(2)).squeeze(2) # (bsz*num_doc*num_ranks,n_prompt,1)->(bsz*num_doc*num_ranks,n_prompt)
                # logger.info(f'shape: {ga_attention_mask.shape}, value: {ga_attention_mask}')
                # logger.info(f'shape: {dense_ib_scores.shape}')
                dense_ib_scores += NEG_INF*(1 - ga_attention_mask) # broadcast, (1,bsz*num_doc*num_ranks,n_prompt)->(bsz*num_ranks,bsz*num_doc*num_ranks,n_prompt)
                # logger.info(f'value: {dense_ib_scores}')
            ...
            if self.model_args.pooling=='hybride':
                ...
                dense_ib_similarities = dense_ib_scores[:,:,0] # (bsz*num_ranks,bsz*num_doc*num_ranks)
                reg_target = torch.zeros_like(dense_ib_similarities,dtype=torch.long) # (bsz*num_ranks,bsz*num_doc*num_ranks)
            else: # for multi-vector (max pooling) or mean
    ...
    def calc_similarities(self,  emb_query, emb_doc): # (bsz,hidden_size), (bsz*num_doc,n_prompt,hidden_size)
        ...
        _,n_prompt,emb_dim = ga_emb_doc.shape # n_prompt is max_doc_length
