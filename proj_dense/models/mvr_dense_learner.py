...
from peach.enc_utils.enc_learners import MVRLearnerMixin
...
class MVRDenseLearner(MVRLearnerMixin):
    ...
    def freeze_encoder(self):
        logger.info("freezing passage encoder parameters ...")
    ...
    def init_prompt(self,prompt_init_path):
        logger.info("initializing prompt parameters ...")
        kmeans = load_pickle(os.path.join(prompt_init_path,'kmeans.pkl'))
        assert self.prefix_encoder[0].weight.data.shape == kmeans.cluster_centers_.shape # (n_prompt,hidden_size)==(n_clusters,n_features)
        scaler = StandardScaler()
        self.prefix_encoder[0].weight.data = torch.tensor(scaler.fit_transform(kmeans.cluster_centers_))
    ...
    def forward(
        ...
        if training_mode is None: # "retrieval_finetune" for train; None for eval (rerank and full rank)
            if is_query:
                ...
                if self.model_args.pooling in ['hybride','last_reg']:
                    return get_representation_tensor(self.encoding_doc(input_ids, attention_mask, ))[:,0,:].unsqueeze(1).contiguous() # (bsz,n_prompt,hidden_size)->(bsz,1,hidden_size)
                else:
    ...
    def calc_xentropy_loss_for_reg(
        ...
        # dict_for_meta["argmax_prompt_index"] = target # (bsz*num_ranks)
