...
class DatasetMacroPassages(Dataset):
    # DATA_TYPE_SET = set(["train", "dev", "test2019", ])
    ...
    def __init__(self, data_type, data_dir, data_names, data_args, tokenizer, add_title=True):
        ...
        if file_exists(offset_file_path):
            ...
            offsets = get_line_offsets(self.data_path, encoding="utf-8") # List[file position of each line]
            save_json(offsets, offset_file_path)
        self.example_list = offsets # List[file position of each line], reading collection.tsv in the 'disk' mode 
    ...
    def __getitem__(self, item):
        with open(self.data_path) as fp:
            fp.seek(self.example_list[item]) #  sets the current file position in a file stream, and returns the new postion.
...
class DatasetFullRankQueries(Dataset):
    ...
    # MSMARCO_PASSAGE_DEV_QRELS_FILENAME = "passage_ranking/qrels.{}.tsv"
    MSMARCO_PASSAGE_DEV_QUERY_FILENAME = "passage_ranking/{}.query.txt"
    # MSMARCO_PASSAGE_COLLECTION_FILENAME = "collection.tsv"
...
class DatasetCustomRerank(Dataset):
    # DATA_TYPE_SET = set(["dev", "test2019", ])
    ...
    # default
    MSMARCO_PASSAGE_DEV_TOP1000_FILENAME = "passage_ranking/top1000.{}"
    ...
    def __init__(
        ...
        # q relevance
        if self.data_type == "test2019" or self.data_type == "test2020":
            qrels = load_tsv(
        ...
        if filepath_dev_qid2top1000pids is None or filepath_dev_qid2top1000pids == "none":
            ...
            if file_exists(qid2top1000pids_file_path):
                ...
                qid2top1000pids = collections.defaultdict(list) # Dict[qid:List[pid]]
            ...
            elif filepath_dev_qid2top1000pids.endswith("trec"): # /dev_search_result_hits100.trec
                # load trec files
        ...
        qid2top1000pids = dict((int(qid), top1000pids) for qid, top1000pids in qid2top1000pids.items()) # Dict[qid:List[pid]]

        self.qid_list = list(sorted(self.qid2query.keys()))[:num_dev] # List[qid] only first 6980

        self.example_list = [] # List[Tuple(qid,pid,label)]
    ...
    def __init__(self, data_type, data_dir, load_type, data_args, tokenizer, num_dev=None, add_title=True):
        ...
        if file_exists(offset_file_path):
            ...
            for idx, (qd_pair, offset) in enumerate(zip(qd_pairs, offsets)):  # grouped by "qid"
                assert len(qd_pair) == 4
                qid, pid = int(qd_pair[0]), int(qd_pair[1])
                qid2offsets[qid].append(offset) # Dict[qid:List[offset]]
            save_json(qid2offsets, offset_file_path)
        self.data_path = msmarco_passage_dev_top1000_path
        self.qid2offsets = qid2offsets # Dict[qid:List[offset]]
        ...
        self.qid_list = qid_list[:num_dev] # 6980
        ...
        self.example_list = [] # List[offsets] ordered in self.qid_list
        for qid in self.qid_list:
