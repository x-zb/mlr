...
class MetricRanking(datasets.Metric):
    ...
    def _compute(self, *, predictions=None, references=None, group_labels=None, num_examples=None, **kwargs):
        ...
        recall_q_top50 = set()
        recall_q_top100 = set()
        recall_q_top1000 = set()
        recall_q_all = set()

        for qid in qid2predictions:
            qid2predictions[qid].sort(key=lambda e: e[0], reverse=True) # sort according to predictions in descending order
            predictions = qid2predictions[qid]
            ranking.append(0) # if the first MaxMRRRank predictions does not contain the positive doc (ref==1), ranking=0, else ranking=the actual rank (i+1)
        ...
        recall_top5 = len(recall_q_top5) * 1.0 / num_queries
        recall_top10 = len(recall_q_top10) * 1.0 / num_queries
        ...
        recall_top1000 = len(recall_q_top1000) * 1.0 / num_queries
