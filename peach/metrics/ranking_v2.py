...
class MetricRankingV2(datasets.Metric):
    ...
    def _compute(
        ...
        # predictions = scores by the retriever
        # references = pids
        # group_labels = metric_ranking.qids_list, indicating which query the corresponding pids and scores belong to. 
        if num_examples is not None:
            predictions = predictions[:num_examples]
            references = references[:num_examples]
            group_labels = group_labels[:num_examples]
        # read qrels
        qrels = dict() # {qid: {pid: gold_score, ...}, ...}
        idx = 0
        with open(qrels_path) as fp:
            for line in fp:
                data = line.strip().split("\t")
                if len(data)==1: # for qrels.test2019.tsv
                    data = data[0].split(" ")
        ...
        results = dict() # {qid: {pid: gold_score, ...}, ...}
        ...
        if len(qrels) != len(results): # num of queries
        ...
        res_metrics.update(calc_mrr(qrels, results, [10, 100])) # {"MRR@{k}":float}
        res_metrics.update(calc_mrr_official(qrels, results, [10, 100])) # {"MRR_official@{k}":float}
        res_metrics.update(calc_recall_rocketqa(qrels, results, [1, 5, 10, 20, 50, 100, 500, 1000, 5000])) # {"recall@{k}":float}
        res_metrics.update(calc_recall_official(qrels, results, [1, 5, 10, 20, 50, 100, 500, 1000, 5000])) # {"recall_official@{k}":float}
        res_metrics.update(calc_ndcg(qrels, results, [10, 100])) # {"NDCG@{k}":float}
        return res_metrics 
