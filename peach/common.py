...
def get_line_offsets(path, encoding="utf-8"):
    offset_list = []
    with open(path, encoding=encoding, mode="r") as fp: # collection.tsv's format: pid \t text
        line = True
        while line:
            begin_tell = fp.tell() # returns the current file position
            line = fp.readline()
            if line.strip():
                offset_list.append(begin_tell)
    return offset_list # List[file position of each line]
