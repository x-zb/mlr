...
def preproc_inputs(
    ...
    org_doc_shape = input_ids.shape # (bsz,num_doc,doc_len) for train set; (bsz,doc_len) for dev set
    org_query_shape = input_ids_query.shape if input_ids_query is not None else None # (bsz,query_len)
    if input_ids.ndim >= 3:
        assert input_ids.ndim == 3
        high_dim_flag = True
        num_docs = input_ids.shape[1]

        input_ids = input_ids.view(-1, input_ids.shape[-1]) # (bsz*num_doc,doc_len)
        attention_mask = attention_mask.view(-1, attention_mask.shape[-1]) # (bsz*num_doc,doc_len)
        if token_type_ids is not None:
            token_type_ids = token_type_ids.view(-1, token_type_ids.shape[-1]) # (bsz*num_doc,doc_len)
        if position_ids is not None:
            position_ids = position_ids.view(-1, position_ids.shape[-1]) # (bsz*num_doc,doc_len)

        if input_ids_query is not None:
            input_ids_query = input_ids_query.view(-1, input_ids_query.shape[-1]) # (bsz,query_len)
            attention_mask_query = attention_mask_query.view(-1, attention_mask_query.shape[-1]) # (bsz,query_len)

            if token_type_ids_query is not None:
                token_type_ids_query = token_type_ids_query.view(-1, token_type_ids_query.shape[-1]) # (bsz,query_len)
            if position_ids_query is not None:
                position_ids_query = position_ids_query.view(-1, position_ids_query.shape[-1]) # (bsz,query_len)
    else:
        high_dim_flag = False
        num_docs = 1
