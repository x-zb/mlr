...
def load_trec(trec_file):
    ...
    qid2results = dict()  # {qid: {pid1: score1, pid2: score2, ...}, ...}
    current_rank = -1 # added
    with open(trec_file) as fp:
        for idx_line, line in enumerate(fp):
            ...
            if qid not in qid2results:  # initialize, there will be {args.hits_num} same qids 
                qid2results[qid] = collections.OrderedDict()
                assert rank==1 # added
            else: # added
                assert rank>current_rank # added
            current_rank = rank # added

            qid2results[qid][pid] = score
    return qid2results # {qid: {pid1: score1, pid2: score2, ...}, ...}
...
def calculate_metrics_for_qid2results(qid2results, qrels_path):
    # qid2results {qid: {pid1: score1, pid2: score2, ...}, ...}
    import datasets
    ...
    ) # .compute()'s args `predictions` and `references` do not need to be passed; their values were stored when .add_batch() was called.
    return eval_metrics 
    # {"QueriesRanked":int, "MRR@{k}":float, "MRR_official@{k}":float, "recall@{k}":float, "recall_official@{k}":float, "NDCG@{k}":float}


def calculate_metrics_for_qid2results_with_qrefs(qid2results, qrefs_path):
...
def load_qrefs(qaref_file_path):
    #
    qid2refs = dict()
    with open(qaref_file_path) as fp:
        for line in fp:
            data = line.strip("\n").split("\t")
            assert len(data) >= 2
            qid2refs[int(data[0])] = data[1:]
    return qid2refs


def evaluate_trec_file(trec_file_path, qrels_file_path):
    qid2results = load_trec(trec_file_path) # {qid: {pid1: score1, pid2: score2, ...}, ...}
    if os.path.exists(qrels_file_path) and os.path.isfile(qrels_file_path):
        eval_metrics = calculate_metrics_for_qid2results(qid2results, qrels_file_path) # {"QueriesRanked":int, "MRR@{k}":float, "MRR_official@{k}":float, "recall@{k}":float, "recall_official@{k}":float, "NDCG@{k}":float}
    else:
